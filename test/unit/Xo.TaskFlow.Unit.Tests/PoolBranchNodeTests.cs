...
public class PoolBranchNodeBuilderTests : BaseBranchNodeTests
{
	...
	[Fact]
	public async Task PoolBranchNodeTest()
	{
		// Arrange
		var cancellationToken = this.CancellationTokenFactory();

		var next = this._NodeFactory.Create()
		...
		var data = (msg as BaseMsg<IMsg?[]>)!.GetData();
		int i = (data[0] as BaseMsg<int>)!.GetData();
	...
	public async Task PoolBranchNodeBuilderTest()
	{
		...
		var root = this._NodeBuilderFactory.Pool()
										.AddNext(linked)
